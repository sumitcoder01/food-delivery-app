...
export default function Carousel({ filter, setFilter }) {
    ...
    return (
        <div>
            <div id="carouselExampleAutoplaying" className="carousel slide" data-bs-ride="carousel" style={{ height: "450px" }}>
                <div className="carousel-inner" style={{ zIndex: '-10' , height:"400px" }}>
                    {images.map((image, index) => (
                        <div className={`${index === 0 ? 'carousel-item active' : 'carousel-item'} overflow-y-hidden`} key={index}>
                            <img src={image} className="w-100 object-fit-fill opacity-50 rounded float-start" alt={`Slide ${index + 1}`} />
                        </div>
                    ))}
                </div>
                <div className="carousel-caption d-block text-center" style={{ zIndex: '10' }}>
