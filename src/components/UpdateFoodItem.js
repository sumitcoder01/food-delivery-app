...
export default function UpdateFoodItem({ updateFoodItem, item, onClose }) {
    ...
    const handleOnSubmit = (e) => {
        e.preventDefault();
        updateFoodItem({data:formData,id:item._id});
        onClose();
    }
      useEffect(()=>{
         if(formData.options[0]?.half){
            setOptionsCategory(true);
         }
         else{
            setOptionsCategory(false);
         }
